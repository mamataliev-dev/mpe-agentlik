<template>
  <div>
    ...
    <div v-if="data?.others?.length > 0" class="flex flex-col gap-y-4 lg:gap-y-8 mt-10 lg:mt-20">
      <h3 class="page-subtitle">{{ translations['postPage.related_post'] }}:</h3>

      <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        <TheCard v-for="item in data.others" :link="`/agentlik/jamoatchilik-kengashi/${item.slug}`" :image="item.image" :title="item.title" :date="item.created_at"
...
const { data } = await useAsyncData('communityCouncilId', () => useAgency().getAgency(`/community_council/${route.params.id}`));
const page = usePage();

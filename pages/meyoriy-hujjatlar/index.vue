  ...
  <div class="wrapper page-container">
    <div :class="!isPostPage ? '' : 'lg:w-10/12 mx-auto'">
      ...
      <div class="page-block">
        <SideNav v-show="!isPostPage" class="lg:hidden w-full" :links="sideBarLinks" :title="translations['header.regulatory_documents']" />
        ...
        <div v-if="!isPostPage" class="w-full lg:w-3/12 flex flex-col sticky top-24">
          <SideNav v-show="!isPostPage" class="hidden lg:block" :links="sideBarLinks" :title="translations['header.regulatory_documents']" />
          <SideRate />
        </div>
      </div>
    </div>
...
const { data } = await useAsyncData('documents', () => useDocuments().getDocuments('/documents/categories'));
...
const updateTitle = () => {
  if (data.value?.results?.length > 0) {
    ...
    if (item) {
      ...
      page.setTitle(translations.value["header.regulatory_documents"]);
      useHead({
        title: 'MPE.UZ'
      });
    }

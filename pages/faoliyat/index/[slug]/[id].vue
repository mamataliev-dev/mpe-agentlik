  ...
  <div>
    ...
    <div v-if="data?.others?.filter(item => item.category.slug === route.params.slug)?.length > 0" class="flex flex-col gap-y-4 lg:gap-y-8 mt-10 lg:mt-20">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        <TheCard v-for="item in data.others.filter(item => item.category.slug === route.params.slug)" :link="`/faoliyat/${route.params.slug}/${item.slug}`" :image="item.image" :title="item.title" :date="item.created_at"
...
const { data } = await useAsyncData('activitiesId', () => useActivities().getActivities(`/activities/${route.params.id}`));

  ...
  <div class="wrapper page-container">
    <div :class="!isPostPage ? '' : 'lg:w-10/12 mx-auto'">
      ...
      <div class="page-block">
        <SideNav v-show="!isPostPage" class="lg:hidden w-full" :links="sideBarLinks" :title="translations['header.press_service']" />
        ...
        <div v-if="!isPostPage" class="w-full lg:w-3/12 flex flex-col sticky top-24">
          <SideNav v-show="!isPostPage" class="hidden lg:block" :links="sideBarLinks" :title="translations['header.press_service']" />
          <SideRate />
        </div>
      </div>
    </div>
...
const { data } = await useAsyncData('news', () => useNews().getNews('/news/categories'));
...
const updateTitle = () => {
  if (data.value?.results?.length > 0) {
    ...
    if (item) {
      ...
      page.setTitle(translations.value["header.press_service"]);
      useHead({
        title: 'MPE.UZ'
      });
    }
